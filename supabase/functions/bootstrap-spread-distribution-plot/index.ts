...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const vals = (data ?? []).map((r: Record<string, unknown>) => Number(r[field])).filter((v)=>Number.isFinite(v));
...
const data = [{ type:'histogram', x, nbinsx:${bins}, marker:{ color:'#f59e0b', line:{ color:'#000', width:1 } }, hovertemplate:'Mean Spread: %{x:.4f}<br>Count: %{y}<extra></extra>' }];

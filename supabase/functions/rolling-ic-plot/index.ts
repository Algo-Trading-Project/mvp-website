...
<script>
const data = [{ x: ${JSON.stringify(x)}, y: ${JSON.stringify(y)}, type: 'scatter', mode: 'lines', connectgaps: true, line: { color: '#60a5fa', width: 2 }, hovertemplate: 'Date: %{x}<br>IC (\\u03c1): %{y:.3f}<extra></extra>' }];

...
Deno.serve(async (req) => {
  try {
    ...
    if (!rows.length) {
      return Response.json({ html: '<html><body style="background:#0b1220;color:#e2e8f0;padding:16px">No IC data in selected range.</body></html>', bins: [] });
    }

    const allVals = rows.map(r => Number(r.ic)).filter(v => Number.isFinite(v));

    if (!allVals.length) {
      return Response.json({ html: '<html><body style="background:#0b1220;color:#e2e8f0;padding:16px">No valid IC values in range.</body></html>', bins: [] });
    }

    const mean = allVals.reduce((a, b) => a + b, 0) / allVals.length;
    const variance = allVals.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / allVals.length;
    const std = Math.sqrt(variance);
    const posCount = allVals.filter(v => v > 0).length;
    const pos = posCount / allVals.length;

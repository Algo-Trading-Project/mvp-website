...
Deno.serve(async (req) => {
  try {
    ...
    const { horizon = '1d', direction = 'long', start, end, samples = 10000, bins = 30, width = 980, height = 360 } = await req.json();
...
Plotly.newPlot(el, data, layout, config);
    ...
    return Response.json({ html, summary: { mean: meanOfMeans, ci_lower: lowerBound, ci_upper: upperBound } });
  } catch (e) {
